...
import ItemCollectionCard from "../../../../components/bookCollect/itemCollectionCard";

export default function BookCollectScreen({ navigation, route }) {
  // Auth
  ...
  const merginObject = (formatedCartList) => {
    ...
    if (recurrent) {
      // collectionCartInfo.weekday = recurrentDay;
      collectionCartInfo.weekday = recurrentDay;
      collectionCartInfo.hour = recurrentHour;
      delete collectionCartInfo.date;
      delete collectionCartInfo.horario;
    } else {
      collectionCartInfo.cupom = cupom;
      // collectionCartInfo.horario = collectHour;
  ...
  async function requestCart(completedCart) {
    try {
      console.log("completeCart", completedCart);
      if (recurrent) {
        let response = await api.post(`/coleta_recorrente`, completedCart);
      } else {
        let response = await api.post(
          `/users/${user.id}/coleta`,
          completedCart
        );
      }
      showMessage({
  ...
  return (
    <SafeAreaView style={{ flex: 1 }}>
      <View style={styles.headerMenu}>
        <TouchableOpacity
          onPress={() => navigation.navigate("Confirm Collect")}
        >
          <Feather name="arrow-left" size={25} color={colors.primary} />
        </TouchableOpacity>
        <Text style={styles.headerTitle}>
          {recurrent ? "Coleta Recorrente" : "Agendar Coleta"}
        </Text>
      </View>
      ...
      <View style={styles.background}>
        ...
        >
          <Text style={styles.inputTitle}>Adicionar item à coleta:</Text>
